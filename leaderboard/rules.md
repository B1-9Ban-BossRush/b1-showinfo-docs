...
2. 必须使用九禁连战速通专用“数据实时显示MOD”（简称ShowInfo）进行连战（[下载地址](/quick-start/install.html)）。版本号必须 >=2.0-dev.7。
...
9. PS5 玩家只需连战结束后按 HOME 键展示 PS5 系统界面即可（战斗界面手柄图标不能完全保证游戏画面来自 PS5 主机）。

以下为建议而非强制：

10. 画面上需要显示游戏帧数（任意显示帧数工具均可）。
11. 尽量选择低画质，确保画面流畅不掉帧。
12. 关闭帧生成，设置 120 帧率上限。
13. 建议保留原版录制视频。以防需要后续盯帧分析。
